...
class Page extends React.Component {
    ...
    if (new Date(Date.now()) >= new Date(localStorage.getItem("expiry"))) {
      ...
      await fetch(url, {
        ...
        .then((data) => {
          ...
          this.refs[`ref_${curr_id}`].innerHTML =
            "Successfully added to favourites";
          setTimeout(() => {
            this.refs[`ref_${curr_id}`].style.display = "none";
          }, 2000);
          return;
        });
    }
  ...
  triggerpopup(e) {
    if (this.state.image != null) {
      ...
      this.refs[`ref_${e.currentTarget.id}`].innerHTML =
        "Please upload your previous image to save changes or clear the previous file";
      setTimeout(() => {
        this.refs[`ref_${curr_id}`].style.display = "none";
      }, 2000);
      return;
              ...
              <div className="details">
                <div id="all-details">
                  <div className="name">
                    <div className="date">
                      <em>
                        {item.edit} {new Date(item.dot).toLocaleString()}
                      </em>
                    </div>
                    <div className="content-name">{item.todoname}</div>
